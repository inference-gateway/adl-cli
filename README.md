...
> **Note:** The code examples in this section are **illustrative** - they show the conceptual shape of the generated code but may not match the exact output of the current templates. For the authoritative generated output, refer to the Go templates in `internal/templates/languages/go/` (config.go.tmpl, service.go.tmpl, tool.go.tmpl, main.go.tmpl).
...
> Illustrative example - see `internal/templates/languages/go/config.go.tmpl` for the actual template. The `Config` struct is generated dynamically from `spec.config` entries; the `Environment` field shown below is not generated by the template.
...
type Config struct {
    ...
    // Custom configuration sections (one per spec.config entry, excluding "tools")
...
func NewExportReportTool(
    ...
    return server.NewBasicTool(
        "export_report",
        "Export data and email report",
        map[string]any{ /* schema */ },
        tool.ExportReportHandler,
    )
}

func (t *ExportReportTool) ExportReportHandler(ctx context.Context, args map[string]any) (string, error) {
    // Direct access to email config subsection
    apiKey := t.email.APIKey
    fromAddress := t.email.FromAddress
    provider := t.email.Provider
...
> Illustrative example - see `internal/templates/languages/go/main.go.tmpl` for the actual template.

```go
// In main.go - config subsection is passed directly
exportReportTool := tools.NewExportReportTool(l, databaseSvc, &cfg.Email)
toolBox.AddTool(exportReportTool)
```
...
> Illustrative example - see `internal/templates/languages/go/service.go.tmpl` for the actual template.

```go
type CalendarService interface {
    // TODO: Define the methods for CalendarService service
    // Example:
...
func NewCalendarService(logger *zap.Logger, cfg *config.Config) (CalendarService, error) {
    // TODO: Implement constructor logic for CalendarService
    logger.Info("initializing CalendarService service")
    return &calendarServiceImpl{}, nil
}
```
...
func NewCreateEventTool(logger *zap.Logger, calendar googleCalendar.CalendarService, cache cache.CacheRepository) server.Tool {
