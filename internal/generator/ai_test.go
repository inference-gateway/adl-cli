...
`)
	...
	assertContains(t, body, "anthropics/claude-code-action@v1.0.175", "Claude Code workflow body")
...
`)
	...
	assertContains(t, body, "inference-gateway/infer-action@v0.32.2", "Infer workflow body")
...
`+sandboxYAML)
		...
		assertContains(t, flox, `infer.flake = "github:inference-gateway/cli/v0.148.1"`, "flox manifest")

		dc := readGenerated(t, out, ".devcontainer/devcontainer.json")
		assertContains(t, dc, "install.sh | bash -s -- --version v0.148.1", "devcontainer postCreateCommand")
	})
